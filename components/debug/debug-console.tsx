...
export const DebugConsole = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 hidden max-h-64 w-80 flex-col overflow-hidden rounded-2xl bg-black/80 p-4 text-xs text-white shadow-card md:flex">
      <h4 className="mb-2 font-semibold">Debug Records</h4>
      <div className="flex-1 overflow-y-auto space-y-2">
        {recentJobs.map((job) => (
          <div key={job.id} className="rounded-lg bg-white/10 p-2">
            <p>ID: {job.id}</p>
            <p>Status: {job.state}</p>
            {job.failure_reason && <p>Error: {job.failure_reason}</p>}
          </div>
        ))}
        {!recentJobs.length && <p className="text-white/60">No records</p>}
      </div>
    </div>
  );
};
